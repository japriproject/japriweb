  ...
  useEffect(() => {
    if (noHp.length >= 10) {
      ...
      if (detected) {
        setLoadProduk(true)
        setShowAll(false)
        fetch(`/api/produk?brand=${encodeURIComponent(detected)}&kategori=Pulsa`)
          .then(r => r.ok ? r.json() : [])
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 safe-pb">
      ...
      <div className="flex-1 px-4 py-4 space-y-3.5">
        {/* Input Nomor */}
        <CustomerInput value={noHp} onChange={value => { setNoHp(value); setSelected(null) }} label="Data Pelanggan" />
          ...
          <div className="bg-white rounded-2xl p-4 card-shadow border border-gray-100/80 slide-up">
                ...
                return (
                  <button
                    ...
                    className={`p-3.5 rounded-xl border-2 text-left transition-all btn-press relative overflow-hidden ${
                      active ? 'border-violet-500 bg-violet-50' : 'border-gray-100 bg-gray-50'
                    }`}
                  >
                    {active && (
                    ...
                    <p className={`text-sm font-bold mt-1.5 ${active ? 'text-violet-600' : 'text-gray-500'}`}>
                      {formatRupiah(p.sale)}
                    </p>
                  </button>
                )
            ...
            {produks.length > 6 && (
              <button
                ...
                className="w-full mt-3 py-2.5 text-xs font-bold text-violet-600 bg-violet-50 rounded-xl flex items-center justify-center gap-1.5"
              >
        ...
        {/* Summary */}
        {selected && noHp && (
          <div className="bg-violet-50 border border-violet-200/60 rounded-2xl p-4 space-y-2.5 slide-up">
            ...
            {[['Produk', selected!.name], ['Tujuan', noHp], ['Operator', brand]].map(([k, v]) => (
            ...
            <div className="flex justify-between text-sm pt-2 border-t border-violet-200/60">
              ...
              <span className="font-bold text-violet-600 text-base">{formatRupiah(selected!.sale)}</span>
            </div>
            {saldo < selected!.sale && (
      ...
      <div className="px-4 pb-24">
        <button
          ...
          className="w-full py-4 gradient-primary disabled:bg-gray-100 disabled:shadow-none disabled:text-gray-400 text-white font-bold rounded-2xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-violet-500/25 btn-press disabled:pointer-events-none"
        >
          {loading ? <Loader2 size={18} className="animate-spin" /> : null}
          {loading ? 'Memproses...' : selected ? `Bayar ${formatRupiah(selected?.sale ?? 0)}` : 'Pilih Produk Dulu'}
        </button>
      </div>
