...
export default async function AdminPrabayarPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string; perPage?: string }> }) {
  await requireAdmin()
  ...
  const [countRows, marginRows] = await Promise.all([
    prisma.$queryRaw<Array<{ total: bigint }>>(Prisma.sql`SELECT COUNT(*) AS total FROM pulsa ${where}`),
    prisma.$queryRaw<Array<{ margin: number }>>`SELECT COALESCE(MAX(GREATEST(sale - price, 0)), 0) AS margin FROM pulsa`,
  ])
  ...
  const products = await prisma.$queryRaw<ProductRow[]>(Prisma.sql`
    SELECT id, code, name, kategori, brand, jenis, price, sale, status
    FROM pulsa
    ${where}
    ORDER BY kategori ASC, brand ASC, sale ASC
  ...
  return (
    <div className="mx-auto max-w-7xl p-5 sm:p-8">
      <header className="mb-6">
        ...
        <h1 className="mt-1 flex items-center gap-3 text-2xl font-bold"><Smartphone className="text-slate-500" /> Produk prabayar</h1>
        <p className="mt-2 text-sm text-slate-400">Kelola produk prabayar, sinkron Digiflazz, dan margin jual.</p>
      </header>
      ...
      <div className="mb-5 grid items-start gap-4 lg:grid-cols-[minmax(280px,420px)_1fr]">
        ...
        <ProductActions type="prabayar" defaultMargin={Number(marginRows[0]?.margin ?? 0)} />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[920px] text-left text-sm">
            <thead className="bg-white/[0.03] text-xs uppercase tracking-wide text-slate-500">
              <tr><th className="px-5 py-3">Kode</th><th className="px-5 py-3">Nama</th><th className="px-5 py-3">Kategori</th><th className="px-5 py-3">Brand</th><th className="px-5 py-3">Jenis</th><th className="px-5 py-3">Harga</th><th className="px-5 py-3">Jual</th><th className="px-5 py-3">Status</th></tr>
            </thead>
            <tbody className="divide-y divide-white/[0.06]">
              {products.map((product) => (
                <tr key={product.id} className="hover:bg-white/[0.03]">
                  ...
                  <td className="px-5 py-4 text-slate-400">{product.jenis}</td>
              ...
              {products.length === 0 && <tr><td colSpan={8} className="px-5 py-12 text-center text-slate-500">Produk tidak ditemukan.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      <div className="-mt-px overflow-hidden rounded-b-2xl border border-white/10 bg-slate-900"><DataTablePagination page={page} pageSize={pageSize} totalRows={totalRows} path="/admin/prabayar" query={search} /></div>
    </div>
  )
}
